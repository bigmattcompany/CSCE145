...
public class homework004 {
	...
	public static void display(double[] radius) {
		...
		for (double radius1 : radius) {
			System.out.printf("%.2f ", Math.PI * radius1 * radius1);
		}
		System.out.println();
	}
	public static void uniquedisplay(double[] radius) {
		System.out.println("unique areas: ");
		for (int j = 0; j < radius.length; j++) {
			boolean unique = true;
			int i = 0;
			for (int q = 0; q < radius.length; q++) {
				if (i != q && Math.abs(radius[i] - radius[q]) < 0.0001) {
					unique = false;
					break;
				}
			}
			if (unique) {
				System.out.printf("%.2f ", Math.PI * radius[i] * radius[i]);
			}
		}
		System.out.println();
	}
